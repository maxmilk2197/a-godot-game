...
## 轮齿判定间隔（秒）。
## 系统常把「一个物理轮齿」拆成好几个事件发过来，间隔小于它的连续事件算同一齿，只翻一页；
## 超过它才算新的一齿。太小 => 一齿翻好几页（甩一下跳好几页）；
## 太大 => 连续滚的时候会被吞页。默认 0.08（≈ 每秒最多 12 页，够快）。
@export_range(0.02, 0.3, 0.01) var 轮齿间隔 : float = 0.08
	...
	if event is InputEventMouseButton:
		...
		if event.button_index == MOUSE_BUTTON_WHEEL_UP or event.button_index == MOUSE_BUTTON_WHEEL_DOWN:
			if not event.pressed:
				return
			get_viewport().set_input_as_handled()
			滚轮翻页(event.button_index == MOUSE_BUTTON_WHEEL_DOWN)
			return
...
## 滚轮翻页：**一齿一页**。
##
## 难点在「数齿」：系统常把一个物理轮齿拆成好几个事件发过来，如果每个事件都翻一页，
## 划一下就跳好几页（这个 bug 踩过）。所以按「事件突发」判定 ——
## 与上一个事件间隔小于 轮齿间隔 的算同一齿，只翻一页；间隔够了才算新的一齿。
##
## 而动画期间**照样收**（只更新目标页排队），当前这页放完立刻接着翻，
## 所以快速连滚不会「滚一半被吞掉、得停一下再滚」。
func 滚轮翻页(向下: bool) -> void:
	var 现在 := Time.get_ticks_msec() / 1000.0
	var 向 := 1 if 向下 else -1

	# 间隔够久、或者方向反了 => 新的一齿
	if 现在 - 上次滚轮时刻 > 轮齿间隔 or 向 != 滚轮方向:
		本齿已翻 = false
	上次滚轮时刻 = 现在
	滚轮方向 = 向

...
func _创建气泡(msg: Dictionary) -> Control:
	...
	var text_height = max(20, _估算文字高度(msg["内容"], max_text_width)) + 16
	...
	label.size = Vector2(max_text_width, text_height - 16)
	...
	if is_me:
		...
		label.offset_left = -(bg_width + 0)
		label.offset_right = -24
		label.offset_top = 14
	else:
		bg.position = Vector2(52, 4)
		label.position = Vector2(64, 14)

	row.custom_minimum_size = Vector2(0, bubble_height + 8)
	...
	time_label.size = Vector2(120, 16)
	time_label.position = Vector2(150, bubble_height + 4)
...
func _估算文字宽度(text: String) -> int:
	return min(text.length() * 9 + 20, 200)


func _估算文字高度(text: String, width: int) -> int:
	var chars_per_line = max(1.0, width / 9.0)
